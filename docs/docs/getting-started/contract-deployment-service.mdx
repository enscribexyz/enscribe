...
As detailed in the Naming Smart Contracts section, ENS only allows primary name (reverse resolution) to be set if the contract implements one of the following:
...
If your contract doesn’t support any of the above interfaces and has already been deployed, you can still name it using Enscribe’s [**Naming Existing Contracts feature**](https://app.enscribe.xyz/nameContract).

Refer to the Naming Existing Contracts section for more details on how to.
