...
- `chainName: NetworkName | string` — Network key (e.g., `"mainnet"`, `"sepolia"`, `"base"`, `"optimism"`, `"arbitrum"`, `"scroll"`, `"linea"`).
...
- `transactions: {` - Object containing the transaction hashes for each step of the naming operation.
...
- `chainName: NetworkName | string` - Network key.
- `opType?: string` — Optional operation label for metrics.
- `coinType?: number` — Optional coin type for the target chain.
- `enableMetrics?: boolean` — Enable lightweight metrics (default `false`).
