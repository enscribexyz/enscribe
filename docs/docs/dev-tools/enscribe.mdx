...
#### `isOwnable(address, walletClient)`
```typescript
async isOwnable(
  address: string, 
...
#### `isReverseClaimable(address, walletClient, ensRegistry)`
```typescript
...
#### `isContractOwner(address, walletClient, ensRegistry)`
```typescript
...
#### `getContractAddresses(networkName)`
```typescript
