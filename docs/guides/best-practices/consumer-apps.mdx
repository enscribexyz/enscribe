...
|-------------|-------------|------------------|---------------|
| User-facing | Simple contract<br/>Proxy contracts<br/>Implementation contracts | 1,2 → `<contract-name>.<yourapp>.eth`<br/>3 → `<version>.<contract-name>.<yourapp>.eth` | Enscribe |
| Protocol (if not included within user-facing contract) | Protocol contracts | `<contract-name>.protocol.<yourapp>.eth` | Enscribe |
| Token | Single token<br/>Multiple tokens | 1 → `<token-symbol>.<youorg>.eth`<br/>`token.<yourapp.eth>`<br/>2 → `<token1>.token.<yourapp.eth>` | Enscribe |
| Utilities (optional) | Utility contracts like factory, registry, oracle, escrow, etc contracts | `<util-name>.utils.<yourapp>.eth` | Enscribe |
| Other wallets/operations/misc contracts | Wallet<br/>Admin<br/>Operations<br/>Multisig | `wallet.<yourapp>.eth`<br/>Or<br/>`<role>.<yourapp>.eth` | ENS manager app |
...
**All Decentraland contracts** - https://github.com/decentraland/contracts/blob/gh-pages/addresses.json

We will be picking the important one and categorizing and naming these contracts according to our best practice standard from the below contracts list
