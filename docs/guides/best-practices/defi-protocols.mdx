...
- For L2 chains supporting ENSIP-19, leverage L2 primary naming for these chains
...
|-------------|-------------|------------------|---------------|
| User-facing | Router contracts<br/>Swap interfaces<br/>Periphery contracts<br/>Proxy contracts<br/>Other User facing contracts | `router.<yourprotocol>.eth`<br/>`swap.<yourprotocol>.eth`<br/>Or<br/>`router.<version>.<yourprotocol>.eth`<br/>`swap.<version>.<yourprotocol>.eth` | Enscribe |
| Protocol | Pool implementations<br/>Vault cores<br/>Lending pools<br/>AMM cores | `<contract-name>.protocol.<yourprotocol>.eth`<br/>Or<br/>`<contract-name>.<version>.protocol.<yourprotocol>.eth` | Enscribe |
| Token | Native tokens<br/>LP tokens<br/>Receipt tokens | `<token-symbol>.<yourprotocol>.eth`<br/>or<br/>`<token-name>.token.<yourprotocol>.eth` | Enscribe |
| Utilities | Oracle contracts<br/>Quoter contracts<br/>Factories<br/>Libraries<br/>Multicall | `<contract-name>.utils.<yourprotocol>.eth`<br/>Or<br/>`<contract-name>.<version>.utils.<yourprotocol>.eth` | Enscribe |
| Other wallets/operations | Treasury<br/>Multisig wallets<br/>Admins | `<role>.<yourprotocol>.eth` | ENS Manager App |
