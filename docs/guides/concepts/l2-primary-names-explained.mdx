...
3. You call `setName("alice.base.eth")` on Base reverse registrar
4. **This doesn't work yet** because `alice.base.eth` only has an Ethereum address set

**To fix this:**
1. Set the Base address for `alice.base.eth` by calling:
```solidity
setAddr(
  namehash("alice.eth"), 
...
2. Now both directions work:
   - `alice.base.eth` → `0x1234...5678` on Base (forward)
   - `0x1234...5678` → `alice.base.eth` on Base (reverse)
...
**Ready to set up L2 primary names?** Visit [Enscribe](https://app.enscribe.xyz/nameContract) and give your smart contracts the human-readable identity they deserve.
